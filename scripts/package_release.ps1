# ============================================================
#  DeadlockLingua - 发布包打包脚本
#  按安装需求打包:VPK + 本地桥 + 内置 Node + 自启/启动脚本 + 说明文档
#  产物: dist\DeadlockLingua-v<版本>-win64.zip
...
$Stage = Join-Path $Dist "DeadlockLingua-$Version-win64"
$ZipOut = Join-Path $Dist "DeadlockLingua-$Version-win64.zip"
...
Copy-Item (Join-Path $Root "core\glossary.js") (Join-Path $Stage "core\")
Copy-Item (Join-Path $Root "core\hero_names.js") (Join-Path $Stage "core\")
Copy-Item (Join-Path $Root "core\steamid_enrich.js") (Join-Path $Stage "core\")
...
Copy-Item (Join-Path $Root "StartBridgeSilent.vbs") (Join-Path $Stage "StartBridgeSilent.vbs")
Copy-Item (Join-Path $Root "StopBridge.bat") (Join-Path $Stage "StopBridge.bat")
Copy-Item (Join-Path $Root "RestartBridge.bat") (Join-Path $Stage "RestartBridge.bat")
